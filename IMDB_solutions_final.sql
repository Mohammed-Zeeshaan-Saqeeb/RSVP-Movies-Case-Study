...
WITH movie_rank_on_avgratings AS
(
SELECT m.title,
       r.avg_rating,
...
-- Type your code below:
SELECT COUNT(m.id) as count_of_movies
FROM movie m
     INNER JOIN ratings r 
     ON m.id=r.movie_id
...
SELECT n.name as director_name,
       ...
       DENSE_RANK() OVER(ORDER BY  COUNT(d.movie_id) DESC) as rank_directors
FROM names n 
...
SELECT * FROM top_production_companies 
...
SELECT n.name AS actress_name,
       SUM(r.total_votes) AS total_votes,
       COUNT(rm.movie_id) AS movie_count,
       ROUND(r.avg_rating,2) AS actress_avg_rating,
